...
| **已登录** | 可以看到 **实时发布** 的商品 | 完整（新发布→最新） |

**⚠️ 强烈建议登录闲鱼账号**（用小号），否则监控不到最新商品。
...
在弹出的 Chrome 窗口中访问 `goofish.com`，用手机扫码或短信登录。Cookie 保存在 `~/.chrome-debug-xianyu`，约 7 天有效。
...
3. **CDP Input 事件搜索**：用 `page.click()` + `page.keyboard.type()` 在搜索框中输入关键词（产生 `isTrusted=true` 事件，与真人点击无差别）
4. **两步排序**：点击"新发布"展开下拉菜单 → 点击"最新"获取实时结果
5. **DOM 被动读取**：从渲染后的页面 DOM 中提取商品卡片数据（`a[class*="feeds-item-wrap"]`）
6. **SQLite 去重**：与本地数据库比对，筛出新商品
...
- 建议登录闲鱼账号（小号），未登录只能看到 6 小时前的商品
- Cookie 约 7 天过期，需重新扫码登录
